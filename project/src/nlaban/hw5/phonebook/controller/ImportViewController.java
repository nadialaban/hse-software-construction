...
public class ImportViewController {
		...
		if (selected != null) {
			if (selected.getMessage().equals("* Контакт с таким ФИО уже существует")) {
				...
				try {
					...
					((EditViewController) root.getController())
							.setContact(new Contact(selected.getContact()));
	...
	private void alreadyExistsConflict(ValidationError selected) {
		...
		try {
			FXMLLoader root = new FXMLLoader(
					ExistsConflictViewController.class.getResource(
							"/view/existsConflictView.fxml"));
			Parent rootPane = root.load();

			((ExistsConflictViewController) root.getController()).setOldContact(PhoneBook.getContact(
					selected.getFullName()));
			((ExistsConflictViewController) root.getController()).setFullName(selected.getFullName());
			((ExistsConflictViewController) root.getController())
					.setNewContact(new Contact(selected.getContact()));
